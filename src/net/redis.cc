...
void Redis::upload_files(
  const vector<storage::PutRequest>& upload_requests,
        ...
        [&]( const size_t index ) {
          shared_ptr<redisContext> redis_context {
            redisConnectWithTimeout(
              config_.ip.c_str(), config_.port, redis_timeout ),
            redisFree
          };
          ...
          if ( config_.password.length() ) {
            shared_ptr<redisReply> auth_reply {
              (redisReply*)redisCommand(
                redis_context.get(), "AUTH %s", config_.password.c_str() ),
              freeReplyObject
            };

            if ( auth_reply == nullptr
                 or auth_reply->type == REDIS_REPLY_ERROR ) {
          ...
          for ( size_t first_file_idx = index;
            ...
            while ( response_count != expected_responses ) {
              ...
              if ( redisGetReply( redis_context.get(), (void**)&reply_ptr )
                   != REDIS_OK ) {
        ...
        [&]( const size_t index ) {
          ...
          for ( size_t first_file_idx = index;
            ...
            for ( size_t file_id = first_file_idx;
              ...
              redisAppendCommand(
                redis_context.get(), "GET %s", object_key.c_str() );
              expected_responses++;
            }

...
public:
  GCCArguments( const int argc,
                char* const* argv,
                const bool force_strip = false,
                const bool canonical_paths = false );

  void add_option( const GCCOption option,
                   const std::string& optstr,
                   const char* optargx = nullptr,
                   const char arg_separator = 'X',
  ...
  const std::vector<std::string>& system_include_dirs() const
  {
...
private:
  ...
  std::vector<std::string> get_link_dependencies(
    const std::vector<InputFile>& link_inputs );
  ...
  std::vector<roost::path> scan_build_directory(
...
public:
  ...
  /* static functions */
  static std::pair<std::string, std::string> split_source_name(
  ...
  static std::string stage_output_name( const GCCStage stage,
                                        const std::string basename );

  static std::vector<std::string> parse_linker_output(
    const std::vector<std::string>& linker_args );

  static std::vector<std::string> parse_dependencies_file(
    const std::string& dep_filename,
    const std::string& target_name );

  static std::vector<std::string> generate_dependencies_file(

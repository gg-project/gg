...
{
  target_file.write(
    run( "gcc-7", { "gcc-7", "-dumpspecs" }, {}, true, true, true ) );
}
...
bool has_include_or_incbin( const string& filename )
{
  ...
  while ( getline( fin, line ) ) {
    if ( line.find( ".include" ) != string::npos
         or line.find( ".incbin" ) != string::npos ) {
      return true;
    }
  }
...
string GCCModelGenerator::do_preprocessing( const InputFile& input )
{
  ...
  for ( auto it = arguments_.input_files().rbegin();
        it != arguments_.input_files().rend();
        it++ ) {
    if ( it->index != input.index ) {
      args.erase( args.begin() + it->index );
    } else {
      args.at( input.index ) = input.name;
      args.insert( args.begin() + input.index,
                   language_to_name( input.language ) );
      args.insert( args.begin() + input.index, "-x" );
    }
  }

  args.erase( remove_if( args.begin(),
                         args.end(),
                         []( const string& s ) {
                           return ( s == "-E" or s == "-S" or s == "-c" );
                         } ),
              args.end() );

  args.insert( args.begin(),
               ( operation_mode_ == OperationMode::GCC ) ? "gcc-7" : "g++-7" );
  args.push_back( "-E" );

  args.push_back( "-frandom-seed=" + BEGIN_REPLACE + input.name + END_REPLACE );
  ...
  const auto& temp_output_path_base
    = gg::paths::blob( roost::rbasename( input.name ).string() );
...
vector<roost::path> GCCModelGenerator::scan_build_directory(
  const roost::path& build_dir,
                                          ...
                                          const bool write_placeholder )
{
  ...
  if ( stage != PREPROCESS ) {
    ...
    args.insert( args.begin(),
                 { gcc_data.filename(), "-specs=/__gg__/gcc-specs" } );
    args.push_back( "-o" );
    args.push_back( output );
  }

  bool generate_makedep_file
    = find_if( args.begin(),
               args.end(),
               []( const string& dep ) {
  ...
  switch ( stage ) {
    case PREPROCESS: {
      const vector<string>& include_path
        = ( input.language == Language::C
            or input.language == Language::C_HEADER
            or input.language == Language::ASSEMBLER_WITH_CPP )
            ? c_include_path
            : cxx_include_path;
      ...
      args.push_back( "-frandom-seed=" + BEGIN_REPLACE + input.name
                      + END_REPLACE );
      ...
      if ( arguments_.no_stdinc() ) {
        ...
        if ( input.language == Language::CXX
             or input.language == Language::CXX_HEADER ) {
      ...
      if ( not defer_depgen_ ) {
        ...
        dependencies = generate_dependencies_file( operation_mode_,
                                                   input.name,
                                                   all_args,
                                                   makedep_filename,
                                                   makedep_target );
      ...
      /* We promised that we would add these here, and we lived up to our
         promise... */
      all_args.insert( all_args.begin(),
                       { gcc_data.filename(), "-specs=/__gg__/gcc-specs" } );
      ...
      if ( defer_depgen_ ) {
        ...
        auto process_directory_blueprints = [&]( const string& dir ) {
          ...
          if ( build_dir.initialized()
               and canonical_dir.string().compare(
                     0, build_dir->string().length(), build_dir->string() )
                     == 0 ) {
            return;
          }
        ...
        if ( not arguments_.no_stdinc() ) {
          for ( const string& dir :
                ( arguments_.no_stdincpp() ? c_include_path : include_path ) ) {
            process_directory_blueprints( dir );
          }
        }
        ...
        for ( const auto& tarball : tarballs ) {
          if ( include_tarballs_str.empty() ) {
            include_tarballs_str += tarball;
          } else {
            include_tarballs_str += ":" + tarball;
          }
        }
        ...
        preprocess_envars.emplace_back(
          create_envar( DEPGEN_WORKING_DIRECTORY,
                        roost::current_working_directory().string() ) );
        preprocess_envars.emplace_back(
        ...
        preprocess_envars.emplace_back( create_envar(
          DEPGEN_CC1_HASH, program_data.at( cc1_program ).hash() ) );
      ...
      if ( defer_depgen_ and generate_makedep_file ) {
        ...
        // ThunkPlaceholder deps_placeholder( gg::hash::for_output(
        // generated_thunk_hash, "dependencies" ) ); deps_placeholder.write(
        // makedep_filename );
      }
    ...
    case COMPILE:
      ...
      args.push_back( "-frandom-seed=__GG_BEGIN_REPLACE__" + input.name
                      + "__GG_END_REPLACE__" );
    ...
    case ASSEMBLE:
      if ( first_stage != ASSEMBLE
           and ( arguments_.option_argument( GCCOption::gdwarf_4 ).initialized()
                 or arguments_.option_argument( GCCOption::g ).initialized()
                 or arguments_.option_argument( GCCOption::g0 ).initialized()
                 or arguments_.option_argument( GCCOption::g1 )
                      .initialized() ) ) {
        args.erase( remove_if( args.begin(),
                               args.end(),
                               []( const string& s ) {
                                 return ( s == "-gdwarf-4" or s == "-g"
                                          or s == "-g0" or s == "-g1" );
                               } ),
                    args.end() );
      }
...
void print_gcc_command( const string& command_str )
{
  ...
  size_t line_count
    = static_cast<size_t>( ceil( 1.0 * command_str.length() / window_width ) );
  ...
  , defer_depgen_( options & Options::defer_depgen )
{
  ...
  if ( arguments_.option_argument( GCCOption::print_file_name ).initialized()
       or arguments_.option_argument( GCCOption::dM ).initialized()
       or arguments_.option_argument( GCCOption::dumpmachine ).initialized() ) {
  ...
  for ( const InputFile& input : arguments_.input_files() ) {
    if ( input.source_language == Language::ASSEMBLER_WITH_CPP
         and has_include_or_incbin( input.name ) ) {
      cerr << "\u2570\u257c assembler file has .include or .incbin directive, "
              "executing gcc..."
           << endl;
      exec_original_gcc();
    }
  ...
  if ( arguments_.last_stage() == PREPROCESS
       and ( arguments_.output_filename() == "-"
             or arguments_.output_filename().empty() ) ) {
    ...
    throw runtime_error(
      "outputting to stdout is only allowed for preprocessing stage" );
  }

  if ( arguments_.input_files().size() == 1
       and arguments_.input_files()[0].name == "/dev/null" ) {
    // just run gcc
...
void GCCModelGenerator::generate()
{
  ...
  const size_t source_inputs = count_if(
    input_files.begin(), input_files.end(), []( const InputFile& input ) {
  ...
  for ( size_t input_index = 0; input_index < input_files.size();
        input_index++ ) {
    ...
    const GCCStage input_last_stage
      = ( last_stage == LINK ) ? ASSEMBLE : last_stage;
    ...
    for ( size_t stage_num = first_stage; stage_num <= input_last_stage;
          stage_num++ ) {
      ...
      if ( stage == last_stage ) {
        if ( final_output.empty() ) {
          /* fill in default names based on the original input filename,
             e.g. hello.abc.cc -> hello.s in compile stage, hello.abc.o in
             assembly */
          if ( stage == PREPROCESS ) {
            ...
            final_output = stage_output_name(
              stage, split_source_name( input.name ).first );
          }
        }
        ...
        output_name
          = "output_" + to_string( input_index ) + "_" + to_string( stage_num );
      }

      string last_stage_hash;
      const bool preprocess_this_locally
        = preprocess_locally_ and stage == PREPROCESS and stage != last_stage;

      if ( preprocess_this_locally ) {
        last_stage_hash = do_preprocessing( input );
      } else {
        last_stage_hash = generate_thunk(
          first_stage, stage, input, output_name, stage == last_stage );
      }
      ...
      input.indata = ThunkFactory::Data( input.name,
                                         "nonexistent",
  ...
  if ( last_stage == LINK ) {
    ...
    string last_stage_hash
      = generate_link_thunk( input_files, dependencies, final_output );
...
int main( int argc, char* argv[] )
{
  try {
    ...
    options |= getenv( "GG_GCC_PREPROCESS_LOCALLY" )
                 ? GCCModelGenerator::Options::preprocess_locally
                 : 0;
    options |= getenv( "GG_GCC_ALL_IN_ONE_THUNK" )
                 ? GCCModelGenerator::Options::all_in_one_thunk
                 : 0;
    options |= getenv( "GG_GCC_DEFER_DEPGEN" )
                 ? GCCModelGenerator::Options::defer_depgen
                 : 0;

    GCCModelGenerator gcc_model_generator {
      operation_mode, argc, argv, options
    };

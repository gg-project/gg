...
void generate_thunk( int argc, char* argv[] )
{
  ...
  vector<string> original_args = gg::models::args_to_vector(
    argc, argv, program_data.at( AR ).filename() );
  ...
  while ( true ) {
    const int opt
      = getopt_long( argc, argv, "abiNpxtTdms", long_options, NULL );
  ...
  ThunkFactory::generate( { program_hash( AR ), original_args, {} },
                          data,
                          { program_data.at( AR ) },
                          { { "output", outfile } },
                          {},

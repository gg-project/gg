...
private:
  ...
  static constexpr int watch_flags { IN_MOVE | IN_DELETE | IN_CLOSE_WRITE
...
Watcher::Watcher( roost::path&& root,
                  vector<string>&& filters,
                  string&& sock_path )
  : root_( move( root ) )
  , filters_( move( filters ) )
  , sock_path_( move( sock_path ) )
  ...
  } )
{
  ...
  ipc_socket_.listen();
  cerr << "listening on "
       << "unix://" << sock_path_ << "..." << endl;

  poller_.add_action( { ipc_socket_, Direction::In, [this]() -> ResultType {
...
void Watcher::watch_callback( const inotify_event& event,
...
int main( int argc, char* argv[] )
{
  try {
    ...
    Watcher watcher { roost::canonical( argv[1] ),
                      read_filters( argv[2] ),
                      argv[3] };

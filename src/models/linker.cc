...
vector<string> GCCModelGenerator::get_link_dependencies(
  const vector<InputFile>& link_inputs )
{
  ...
  args.erase( remove( args.begin(), args.end(), string( "-Wl,--as-needed" ) ),
              args.end() );
...
string GCCModelGenerator::generate_link_thunk(
  const vector<InputFile>& link_inputs,
  ...
  const string& output )
{
  ...
  for ( auto const& link_input : link_inputs ) {
    if ( link_input.language == Language::OBJECT
         or link_input.language == Language::SHARED_OBJECT
         or link_input.language == Language::ARCHIVE_LIBRARY ) {

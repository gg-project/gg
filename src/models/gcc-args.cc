...
GCCArguments::GCCArguments( const int argc_orig,
                            char* const* argv_orig,
                            const bool force_strip,
                            const bool canonical_paths )
  : force_strip_( force_strip )
  , canonical_paths_( canonical_paths )
{
  if ( canonical_paths_ ) {
  ...
  constexpr GCCOptionData gcc_options_data[] = {
    ...
    { GCCOption::mpreferred_stack_boundary,
      "mpreferred-stack-boundary",
      required_argument,
      false,
      '=' },
    { GCCOption::mno_tls_direct_seg_refs,
      "mno-tls-direct-seg-refs",
      no_argument,
      false,
      'X' },
    { GCCOption::mregparm, "mregparm", required_argument, false, '=' },
    { GCCOption::print_file_name,
      "print-file-name",
      required_argument,
      false,
      '=' },
  ...
  constexpr size_t option_count
    = sizeof( gcc_options_data ) / sizeof( GCCOptionData );
  constexpr auto gcc_options
    = getopt_options<option_count> { gcc_options_data };

  map<GCCOption, GCCOptionData> options_map;
  for ( size_t i = 0; i < option_count; i++ ) {
    options_map.insert(
      make_pair( gcc_options_data[i].option, gcc_options_data[i] ) );
  }
  ...
  while ( true ) {
    const int opt = getopt_long_only(
      argc, argv, gcc_optstring, gcc_options.list, nullptr );
    ...
    if ( opt == 1 ) {
      ...
      if ( file_lang == Language::NONE ) {
        if ( input_file == "-" and last_stage_.initialized()
             and *last_stage_ == PREPROCESS ) {
    ...
    switch ( opt ) {
      ...
      case 'O':
        add_option( GCCOption::O, "O", optarg, '\0' );
        break;
      case 'D':
        add_option( GCCOption::D, "D", optarg, '\0' );
        break;
      case 'U':
        add_option( GCCOption::U, "U", optarg, '\0' );
        break;
      case 'f':
        add_option( GCCOption::f, "f", optarg, '\0' );
        break;
    ...
    if ( not flag_processed ) {
      switch ( gccopt ) {
        ...
        case GCCOption::S:
          last_stage_
            = ( not last_stage_.initialized() or *last_stage_ >= COMPILE )
                ? COMPILE
                : *last_stage_;
          break;
        ...
        case GCCOption::c:
          last_stage_
            = ( not last_stage_.initialized() or *last_stage_ >= ASSEMBLE )
                ? ASSEMBLE
                : *last_stage_;
          break;
        ...
        default:
          if ( not options_map.count( gccopt ) ) {
            throw runtime_error( "unknown gcc flag: "
...
void GCCArguments::add_option( const GCCOption option,
                               const string& optstr,
                               const char* value,
...
const vector<string>& GCCArguments::extra_infiles( const GCCStage stage )
{
  if ( extra_infiles_.count( stage ) == 0 ) {
    extra_infiles_.emplace(
      piecewise_construct, forward_as_tuple( stage ), forward_as_tuple( 0 ) );
  }

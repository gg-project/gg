                     ...
                     char* argv[] )
{
  ...
  for ( const CLIOption& cli_option : cli_description.options() ) {
    if ( cli_option.long_opt.initialized() ) {
      long_options.push_back( { cli_option.long_opt->c_str(),
                                required_argument,
                                nullptr,
                                cli_option.value } );
    }
  ...
  vector<string> args = models::args_to_vector(
    argc, argv, roost::rbasename( cli_description.target_bin() ).string() );
  ...
  while ( ( opt = getopt_long(
              argc, argv, optstring.c_str(), long_options.data(), NULL ) )
          != -1 ) {
    if ( opt == '?' )
      continue;
  ...
  ThunkFactory::generate( { executable.hash(), args, {} },
                          indata,
                          { executable },
                          outfiles,
                          {},

...
vector<string> execute_thunk( const Thunk& original_thunk )
{
  ...
  //   /* Let's see if we can redudce this thunk to an order one thunk by
  //   updating the infiles */
  ...
  // PREPARING THE ENV
  TempDirectory exec_dir { temp_dir_template + string( "." )
                           + thunk.hash().substr( 0, 10 ) };
  ...
  for ( auto& link : thunk.links() ) {
    roost::symlink( gg::paths::blob( link.second ),
                    exec_dir_path / link.first );
  }

  // EXECUTING THE THUNK
  if ( not sandboxed ) {
    ChildProcess process { thunk.hash(), [thunk, &exec_dir_path]() {
                            CheckSystemCall(
                              "chdir",
                              chdir( exec_dir_path.string().c_str() ) );
  ...
  for ( size_t i = thunk.outputs().size() - 1; i < thunk.outputs().size();
        i-- ) {
  ...
  if ( thunk.output_dir().initialized() ) {
    const roost::path output_dir
      = exec_dir_path / roost::path( thunk.output_dir().get() );
  ...
  try {
    ...
    throw runtime_error( string( "After\n\t" ) + original_thunk.hash()
                         + string( "\nexecuted, it left some files behind in "
...
void do_cleanup( const Thunk& thunk )
{
  ...
  for ( const string& blob : roost::list_directory( gg::paths::blobs() ) ) {
    ...
    if ( ( not roost::is_directory( path ) )
         and infile_hashes.count( blob ) == 0 ) {
      roost::remove( path );
    }
  }
}
  ...
  try {
    ...
    auto check_dep
      ...
      if ( not roost::exists( target_path )
           or roost::file_size( target_path )
                != gg::hash::size( item.first ) ) {
        if ( executables ) {
    ...
    for_each( thunk.values().cbegin(), thunk.values().cend(), check_dep );

    executables = true;
    for_each(
      thunk.executables().cbegin(), thunk.executables().cend(), check_dep );
                    ...
                    const vector<string>& output_hashes )
{
  try {
    ...
    for ( const string& output_hash : output_hashes ) {
      requests.push_back( { gg::paths::blob( output_hash ),
                            output_hash,
...
void usage( const char* argv0 )
{
  cerr << "Usage: " << argv0 << " [options] THUNK-HASH..." << endl
       << endl
       << "Options: " << endl
       << " -g, --get-dependencies  Fetch the missing dependencies from the "
          "remote storage"
       << endl
       << " -p, --put-output        Upload the output to the remote storage"
       << endl
...
int main( int argc, char* argv[] )
{
  try {
    ...
    for ( const string& thunk_hash : thunk_hashes ) {
      ...
      FileDescriptor raw_thunk { CheckSystemCall(
        "open( " + thunk_path + " )", open( thunk_path.c_str(), O_RDONLY ) ) };
      raw_thunk.block_for_exclusive_lock();

      Thunk thunk = ThunkReader::read( thunk_path );

      if ( timelog.initialized() ) {
        timelog->add_point( "read_thunk" );
      }

    ...
    if ( tokens[0] == "push" ) {
      ...
      switch ( tokens[1][0] ) {
        case 'a':
          ...
          break;
        case 'd':
          data.emplace_back( tokens[2] );
          break;
        case 'e':
          executables.emplace_back( tokens[2] );
          break;
        case 'o':
          outputs.emplace_back( tokens[2] );
          break;
        case 'l':
          links.emplace_back( tokens[2], gg::hash::file( tokens.at( 3 ) ) );
          break;

        default:
          throw runtime_error( "invalid push operation: " + tokens[1] );
      }
      ...
      switch ( tokens[1][0] ) {
        ...
        default:
          throw runtime_error( "invalid pop operation: " + tokens[1] );
      }
      ...
      switch ( tokens[1][0] ) {
        case 'f':
          function_hash = tokens[2];
          break;
        default:
          runtime_error( "invalid set operation: " + tokens[1] );
      }
      ...
      switch ( tokens[1][0] ) {
        case 't': { // thunk
          ...
          last_thunk_hash = ThunkFactory::generate<deque>(
            { function_hash.empty() ? executables.at( 0 ).hash()
                                    : function_hash,
              { args.begin(), args.end() },
              {} },
            data,
        ...
        case 'p': { // placeholder
          ...
          ThunkPlaceholder placeholder {
            tokens.at( 2 ) == outputs.front().tag()
              ? last_thunk_hash

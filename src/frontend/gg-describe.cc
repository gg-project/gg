...
int main( int argc, char* argv[] )
{
  try {
    ...
    while ( true ) {
      const int opt
        = getopt_long( argc, argv, "ec", command_line_options, nullptr );
    ...
    if ( not roost::exists( thunk_path ) ) {
      ...
      if ( not roost::exists( thunk_path ) ) {
        ...
        if ( glob( pattern.string().c_str(),
                   GLOB_ERR | GLOB_NOSORT,
                   nullptr,
                   &glob_result )
             == 0 ) {
    ...
    if ( print_command ) {
      cout << command_str( thunk.function().args(), thunk.function().envars() )
           << endl;
    }

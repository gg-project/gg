...
int main( int argc, char* argv[] )
{
  try {
    ...
    shared_ptr<TCPConnection> connection = loop.make_connection<TCPConnection>(
      coordinator_addr,
          ...
          case Message::OpCode::Execute: {
            ...
            loop.add_child_process(
              ...
              [hash = execution_request.hash(), execution_request, &connection](
                ...
                Message message { Message::OpCode::Executed,
                                  protoutil::to_string( execution_response ) };
                connection->enqueue_write( message.str() );
              },

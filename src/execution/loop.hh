...
private:
  ...
  Poller poller_ {};
  std::list<std::tuple<uint64_t, bool, LocalCallbackFunc, ChildProcess>>
    child_processes_ {};
  ...
  typename std::list<std::shared_ptr<Connection<SocketType>>>::iterator
  create_connection( SocketType&& socket );

  template<typename ConnectionType>
  void remove_connection(
    const typename std::list<std::shared_ptr<ConnectionType>>::iterator& it );
...
public:
  ...
  std::shared_ptr<Connection<SocketType>> add_connection(
    SocketType&& socket,
  ...
  std::shared_ptr<ConnectionType> make_connection(
    ...
    const std::function<bool( std::shared_ptr<ConnectionType>, std::string&& )>&
      data_callback,
    const std::function<void()>& error_callback = []() {},
    const std::function<void()>& close_callback = []() {} );
  ...
  uint64_t make_listener(
    const Address& address,
    const std::function<bool( ExecutionLoop&, TCPSocket&& )>&
      connection_callback );

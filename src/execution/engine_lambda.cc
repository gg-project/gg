    ...
    [this]( const uint64_t id,
      ...
      if ( http_response.status_code() != "200" ) {
        if ( http_response.status_code() == "429"
             or ( http_response.status_code() == "500"
                  and http_response.has_header( "x-amzn-ErrorType" )
                  and http_response.get_header_value( "x-amzn-ErrorType" )
                        == "ServiceException" ) ) {
      ...
      ExecutionResponse response
        = ExecutionResponse::parse_message( http_response.body() );
      ...
      switch ( response.status ) {
        case JobStatus::Success: {
          if ( response.thunk_hash != thunk_hash ) {
            ...
            throw runtime_error( "expected output for " + thunk_hash
                                 + ", got output for " + response.thunk_hash );
          }

          for ( const auto& output : response.outputs ) {
            gg::cache::insert(
              gg::hash::for_output( response.thunk_hash, output.tag ),
              output.hash );
          ...
          gg::cache::insert( response.thunk_hash,
                             response.outputs.at( 0 ).hash );
          ...
          success_callback_( response.thunk_hash,
                             move( thunk_outputs ),
...
bool AWSLambdaExecutionEngine::can_execute(
  const gg::thunk::Thunk& thunk ) const
{
  return thunk.infiles_size() < 230_MiB;
}

float AWSLambdaExecutionEngine::compute_cost(

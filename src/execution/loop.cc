  ...
  , signal_fd_( signals_ )
{
  ...
  poller_.add_action( Poller::Action(
    ...
    [&]() {
      return ( child_processes_.size() > 0 or connections_.size() > 0
               or ssl_connections_.size() > 0 );
    } ) );
}
...
{
  return ssl_connections_.emplace(
    ssl_connections_.end(), make_shared<SSLConnection>( move( socket ) ) );
}
  ...
  const function<void()>& close_callback )
{
  ...
  poller_.add_action( Poller::Action(
    ...
    Direction::Out,
    [connection]() {
      string::const_iterator last_write = connection->socket_.write(
        connection->write_buffer_.begin(), connection->write_buffer_.cend() );

      connection->write_buffer_.erase(
        0, last_write - connection->write_buffer_.cbegin() );
  ...
  const function<void()>& close_callback )
{
  const auto connection_it
    = create_connection<NBSecureSocket>( move( socket ) );
  ...
  const function<void()>& close_callback )
{
  ...
  return add_connection<TCPSocket>(
    move( socket ), data_callback, error_callback, close_callback );
}
  ...
  const function<void()>& close_callback )
{
  ...
  socket.set_blocking( false );
  socket.connect_nonblock( address );
  NBSecureSocket secure_socket { move(
    ssl_context_.new_secure_socket( move( socket ) ) ) };
  secure_socket.connect();

  return add_connection<NBSecureSocket>(
    move( secure_socket ), data_callback, error_callback, close_callback );
}
...
{
  child_processes_.emplace_back( current_id_,
                                 throw_if_failed,
                                 callback,

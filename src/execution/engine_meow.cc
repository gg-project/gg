...
void MeowExecutionEngine::init( ExecutionLoop& exec_loop )
{
  exec_loop.make_listener(
    { "0.0.0.0", listen_addr_.port() },
    [this]( ExecutionLoop& loop, TCPSocket&& socket ) {
      cerr << "[meow] Incoming connection: " << socket.peer_address().str()
           << endl;
      ...
      auto connection = loop.add_connection<TCPSocket>(
        move( socket ),
        [message_parser, id = current_id_, this]( shared_ptr<TCPConnection>,
                                                  string&& data ) {
          ...
          while ( not message_parser->empty() ) {
            ...
            switch ( message.opcode() ) {
              ...
              case Message::OpCode::Executed: {
                ...
                // cerr << "[meow:worker@" << id << ":executed] " << thunk_hash
                // << endl;

                for ( const auto& output : execution_response.outputs() ) {
                  gg::cache::insert(
                    gg::hash::for_output( thunk_hash, output.tag() ),
                    output.hash() );
                ...
                gg::cache::insert( thunk_hash,
                                   execution_response.outputs( 0 ).hash() );
                ...
                for ( auto& output : execution_response.outputs() ) {
                  thunk_outputs.emplace_back( move( output.hash() ),
                                              move( output.tag() ) );
                }
        ...
        [this]() { throw runtime_error( "a lambda died" ); } );
  ...
  cerr << "[meow] Listening for incoming connections on " << listen_addr_.str()
       << endl;
}
...
void MeowExecutionEngine::prepare_lambda( Lambda& lambda, const Thunk& thunk )
{
  ...
  for ( const auto& item :
        join_containers( thunk.values(), thunk.executables() ) ) {
    if ( not lambda.objects.count( item.first ) /* XXX and
         not gg::remote::is_available( item.first ) */ ) {
      lambda.connection->enqueue_write(
        meow::create_put_message( item.first ).str() );
    }
  ...
  lambda.connection->enqueue_write(
    meow::create_execute_message( thunk ).str() );
  ...
  switch ( s ) {
    ...
    case SelectionStrategy::MostObjects: {
      ...
      if ( selected_lambda != numeric_limits<size_t>::max() ) {
        cerr << "picked a lambda with " << max_common_size << " bytes in common"
             << endl;
...
void MeowExecutionEngine::force_thunk( const Thunk& thunk, ExecutionLoop& loop )
{
  ...
  if ( free_lambdas_.size() > 0 ) {
    ...
    const uint64_t picked_lambda
      = pick_lambda( thunk, SelectionStrategy::MostObjects );

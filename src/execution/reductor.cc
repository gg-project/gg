...
void Reductor::print_status() const
{
  ...
  if ( duration_cast<milliseconds>( this_display - last_display ).count()
       > 33 ) {
    last_display = this_display;

    ostringstream data;

    data << color_reset << "in queue: " << BOLD << COLOR_YELLOW << setw( 5 )
         << left << job_queue_.size() << color_reset;

    for ( auto& ee : exec_engines_ ) {
      data << " " << ee->label() << " (" << ee->max_jobs() << "): " << BOLD
           << COLOR_RED << setw( 5 ) << left << ee->job_count() << color_reset;
    }

    data << " done: " << BOLD << COLOR_GREEN << setw( 5 ) << left
         << finished_jobs_ << color_reset << " remaining: " << BOLD
         << COLOR_DEFAULT << dep_graph_.size();
  ...
  , storage_backend_( move( storage_backend ) )
{
  ...
  auto success_callback = [this]( const string& old_hash,
  ...
  auto failure_callback
    = [this]( const string& old_hash, const JobStatus failure_reason ) {
        switch ( failure_reason ) {
          ...
          /* for all of the following cases, except default, we will push the
          failed job back into the queue */
          ...
          case JobStatus::FetchDependenciesFailure:
            print_gg_message( "warning",
                              "fetching the dependencies failed: " + old_hash );
            break;

          case JobStatus::UploadOutputFailure:
            print_gg_message( "warning",
                              "uploading the output failed: " + old_hash );
            break;
...
vector<string> Reductor::reduce()
{
  while ( true ) {
    while ( not job_queue_.empty() ) {
      ...
      while ( true ) {
        auto temp_cache_entry = gg::cache::check(
          cache_entry.initialized() ? cache_entry->hash : thunk_hash );
        ...
        if ( exec_state == CANNOT_BE_EXECUTED ) {
          for ( auto& fallback_engine : fallback_engines_ ) {
            if ( fallback_engine->can_execute( thunk ) ) {
              if ( fallback_engine->job_count()
                   >= fallback_engine->max_jobs() ) {
    ...
    const auto poll_result = exec_loop_.loop_once(
      timeout_check_interval_ == 0s ? -1 : timeout_check_interval_.count() );
    ...
    if ( timeout_check_interval_ != 0s and clock_now >= next_timeout_check_ ) {
      ...
      if ( count > 0 ) {
        print_gg_message( "info",
                          "duplicating " + to_string( count ) + " job"
                            + ( ( count == 1 ) ? "" : "s" ) );
      }
    }
...
void Reductor::upload_dependencies() const
{
  ...
  auto upload_time = time_it<milliseconds>( [&upload_requests, this]() {
    storage_backend_->put(
      upload_requests, [this]( const storage::PutRequest& upload_request ) {
...
void Reductor::download_targets( const vector<string>& hashes ) const
{
  ...
  cerr << "\u2198 Downloading output file" << plural << " ("
       << format_bytes( total_size ) << ")... ";
  auto download_time = time_it<milliseconds>( [&download_requests, this]() {

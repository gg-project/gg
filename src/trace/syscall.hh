...
constexpr int SYSCALL_ARG_REGS[]
...
#define STACKTRACE_CAPTURE_ON_ENTER                                            \
  01000 /* Capture stacktrace on "entering" stage */
#define TRACE_INDIRECT_SUBCALL                                                 \
  02000 /* Syscall is an indirect socket/ipc subcall. */
#define COMPAT_SYSCALL_TYPES                                                   \
  04000                     /* A compat syscall that uses compat types. */
...
struct ArgumentInfo
{
  ...
  bool is_readable_string() const
  {
    return dir != ARGUMENT_DIR_OUT
           and ( type == typeid( char* ) or type == typeid( const char* ) );
  }
};
...
public:
  SystemCallSignature( const long number,

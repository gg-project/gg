...
{
  return reinterpret_cast<T>( ptrace(
    PTRACE_PEEKUSER, pid_, sizeof( long ) * SYSCALL_ARG_REGS[argnum], NULL ) );
}
...
template<typename T>
void SystemCallInvocation::set_syscall_arg( const uint8_t argnum,
                                            const T& value ) const
{
  ptrace(
    PTRACE_POKEUSER, pid_, sizeof( long ) * SYSCALL_ARG_REGS[argnum], &value );
}
                                            ...
                                            const string& value ) const
{
  if ( value.length() >= PATH_MAX ) {
    throw runtime_error(
      "maximum string length for set_syscall_arg is PATH_MAX" );
  }

  char* stack_addr
    = (char*)ptrace( PTRACE_PEEKUSER, pid_, sizeof( long ) * RSP, 0 );
  ...
  ptrace( PTRACE_POKEUSER,
          pid_,
          sizeof( long ) * SYSCALL_ARG_REGS[argnum],
          str_addr );
}
...
template<>
string Argument::value() const
{
  return value_.string_val.get();
}
template<>
long Argument::value() const
{
  return value_.long_val.get();
}
template<>
int Argument::value() const
{
  return (int)value_.long_val.get();
}
...
void SystemCallInvocation::set_argument( uint8_t argnum, const string value )
{
  ...
  if ( arguments_.initialized() ) {
    arguments_->at( argnum ).set_value(
      value ); /* XXX what about the long_val? */
  }
}
...
template void SystemCallInvocation::set_argument( uint8_t argnum,
                                                  const string value );

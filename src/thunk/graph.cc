                                  ...
                                  const vector<ThunkOutput>& outputs )
{
  ...
  for ( const string& referencing_thunk_hash :
        referencing_thunks_.at( old_hash ) ) {
...
Optional<unordered_set<string>> ExecutionGraph::force_thunk(
  ...
  vector<ThunkOutput>&& original_outputs )
{
  ...
  for ( const string& referencing_thunk_hash :
        referencing_thunks_.at( actual_new_hash ) ) {
    Thunk& referencing_thunk = thunks_.at( referencing_thunk_hash );

    if ( referencing_thunk.can_be_executed() ) {
      const string referencing_thunk_new_hash
        = ThunkWriter::write( referencing_thunk );
...
unordered_set<string> ExecutionGraph::order_one_dependencies(
  const string& input_hash ) const
{
...
{
  return updated_hashes_.count( original_hash )
           ? updated_hashes_.at( original_hash )
           : original_hash;
}

string ExecutionGraph::original_hash( const string& updated_hash ) const
{
  return original_hashes_.count( updated_hash )
           ? original_hashes_.at( updated_hash )
           : updated_hash;
}

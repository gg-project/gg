  ...
  for ( const DataItem& item : data ) {
    switch ( hash::type( item.first ) ) {
      case ObjectType::Value:
        values_.emplace( item );
        break;
      case ObjectType::Thunk:
        thunks_.emplace( item );
        break;
    }
  }
...
int Thunk::execute() const
{
  ...
  auto replace_data_placeholder = []( string& str ) -> void {
    if ( regex_search( str, DATA_PLACEHOLDER_REGEX ) ) {
      string new_path = regex_replace(
        str, DATA_PLACEHOLDER_REGEX, gg::paths::blob( "$1" ).string() );
      swap( str, new_path );
    }
  };
  ...
  if ( ( retval = ezexec(
           gg::paths::blob( function_.hash() ).string(), args, envars ) )
       < 0 ) {
...
protobuf::RequestItem Thunk::execution_request( const Thunk& thunk )
{
  ...
  string base64_thunk;
  StringSource s( ThunkWriter::serialize( thunk ),
                  true,
...
protobuf::Thunk Thunk::to_protobuf() const
{
  ...
  *thunk_proto.mutable_function() = function_.to_protobuf();
  ...
  for ( const auto& h : values_ ) {
    thunk_proto.add_values( data_to_string( h ) );
  }
...
string Thunk::hash() const
{
  if ( not hash_.initialized() ) {
    hash_.reset(
      gg::hash::compute( ThunkWriter::serialize( *this ), ObjectType::Thunk ) );
  }
...
{
  const string combined_hashes
    = accumulate( executables_.begin(),
                  executables_.end(),
                  string {},
                         ...
                         const vector<ThunkOutput>& outputs )
{
  ...
  for ( const auto& output : outputs ) {
    ...
    const string new_hash
      = ( first_output or hash::type( output.hash ) == ObjectType::Value )
          ? output.hash
...
unordered_map<string, Permissions> Thunk::get_allowed_files() const
{
  unordered_map<string, Permissions> allowed_files;

  for ( const DataItem& item : values_ ) {
    allowed_files[gg::paths::blob( item.first ).string()]
      = { true, false, false };
  }

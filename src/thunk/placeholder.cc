...
{
  const char* header
    = ( type == Type::LinkerScript ) ? LIBRARY_DIRECTIVE : SHEBANG_DIRECTIVE;
...
void ThunkPlaceholder::write( const string& filename ) const
{
  ...
  if ( extension == "so" or extension == "a" or extension == "o"
       or extension == "s" or extension == "S" or extension == "sho"
       or regex_match( filename, match, so_pattern ) ) {
...
Optional<ThunkPlaceholder> ThunkPlaceholder::read( const string& filename )
{
  ...
  if ( strncmp( buffer, SHEBANG_DIRECTIVE, sizeof( SHEBANG_DIRECTIVE ) - 1 )
         != 0
       and strncmp( buffer, LIBRARY_DIRECTIVE, sizeof( LIBRARY_DIRECTIVE ) - 1 )
             != 0 ) {
    return {};
  }
...
{
  static_assert(
    sizeof( SHEBANG_DIRECTIVE ) == sizeof( LIBRARY_DIRECTIVE ),
    "length of shebang directive and library directive must be equal" );

  const string header
    = fd.read_exactly( sizeof( SHEBANG_DIRECTIVE ) - 1, true );
  return ( header.compare(
             0, sizeof( SHEBANG_DIRECTIVE ) - 1, SHEBANG_DIRECTIVE )
           == 0 )
         or ( header.compare(
                0, sizeof( LIBRARY_DIRECTIVE ) - 1, LIBRARY_DIRECTIVE )
              == 0 );
}

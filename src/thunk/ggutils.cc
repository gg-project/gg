...
roost::path get_gg_dir()
{
  ...
  if ( envar == nullptr ) {
    ...
    do {
      ...
      if ( roost::exists( path_candidate )
           and roost::is_directory( path_candidate ) ) {
    ...
    throw runtime_error(
      "GG_DIR not set, and could not find .gg in any of parent dirs" );
  }
...
{
  const static roost::path reductions_path
    = get_inner_directory( "reductions" );
...
roost::path hash_cache()
{
  const static roost::path hash_cache_path
    = get_inner_directory( "hash_cache" );
...
{
  const static roost::path include_cache_path
    = get_inner_directory( "include_cache" );
                              ...
                              const struct stat& stat_entry )
{
  const string cache_key = to_string( stat_entry.st_dev ) + "-"
                           + to_string( stat_entry.st_ino ) + "-"
                           + roost::rbasename( filename ).string();
...
Optional<ReductionResult> check( const string& thunk_hash )
{
  ...
  FileDescriptor cache_entry { CheckSystemCall(
    "open( " + reduction.string() + " )",
...
string compute( const string& input, const ObjectType type )
{
  ...
  output_sstr << to_underlying( type ) << ret << setfill( '0' ) << setw( 8 )
              << hex << input.length();
...
string file_force( const roost::path& path, Optional<ObjectType> type )
{
  ...
  if ( not type.initialized() ) {
    type = ( contents.size() >= thunk::MAGIC_NUMBER.size()
             and contents.compare(
                   0, thunk::MAGIC_NUMBER.size(), gg::thunk::MAGIC_NUMBER )
                   == 0 )
             ? ObjectType::Thunk
             : ObjectType::Value;
  }
...
string file( const roost::path& path, Optional<ObjectType> type )
{
  ...
  const auto cache_entry_path
    = gg::paths::hash_cache_entry( path.string(), file_stat );
  ...
  roost::atomic_create(
    to_string( file_stat.st_size ) + " " + to_string( file_stat.st_mtim.tv_sec )
      + " " + to_string( file_stat.st_mtim.tv_nsec ) + " "
      + to_string( file_stat.st_ctim.tv_sec ) + " "
      + to_string( file_stat.st_ctim.tv_nsec ) + " " + computed_hash,
    cache_entry_path );
...
ObjectType type( const string& hash )
{
  ...
  switch ( hash[0] ) {
    case 'T':
      return ObjectType::Thunk;
    case 'V':
      return ObjectType::Value;
    default:
      throw runtime_error( "invalid hash type" );
  }
}

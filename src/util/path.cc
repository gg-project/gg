...
Directory::Directory( const Directory& parent, const string& path )
  : fd_( CheckSystemCall(
    ...
    openat( parent.num(), path.c_str(), O_DIRECTORY | O_CLOEXEC ) ) )
{}
...
void symlink( const path& target, const path& linkpath )
{
  CheckSystemCall(
    "symlink",
    ::symlink( target.string().c_str(), linkpath.string().c_str() ) );
}
...
void copy_then_rename( const path& src,
                       const path& dst,
                       const bool set_mode,
                       const mode_t target_mode )
{
  /* read input file into memory */
  FileDescriptor src_file { CheckSystemCall(
    "open (" + src.string() + ")", open( src.string().c_str(), O_RDONLY ) ) };
  ...
  atomic_create(
    contents, dst, true, set_mode ? target_mode : src_info.st_mode );
}

path operator/( const path& prefix, const path& suffix )
{
  if ( ( not prefix.string().empty() and prefix.string().back() == '/' )
       or ( not suffix.string().empty() and suffix.string().front() == '/' ) ) {
                                  ...
                                  const vector<string>::const_iterator& end )
{
  ...
  create_directories_relative(
    Directory( parent_directory, *begin ), begin + 1, end );
}
...
void create_directories( const path& pathn )
{
  ...
  create_directories_relative(
    Directory( "." ), components.begin(), components.end() );
}
...
bool remove_at( const Directory& parent_directory,
                const path& pathn,
...
{
  CheckSystemCall(
    "rename", ::rename( oldpath.string().c_str(), newpath.string().c_str() ) );
}
...
string readlink( const path& pathn )
{
  ...
  ssize_t len = CheckSystemCall(
    "readlink",
    ::readlink( pathn.string().c_str(), result, sizeof( result ) ) );
  result[len] = '\0';

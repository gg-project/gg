...
public:
  ...
  uint16_t be16( void ) const { return be16toh( extract_value<uint16_t>() ); }

  uint64_t be32( void ) const { return be32toh( extract_value<uint32_t>() ); }

  uint64_t be64( void ) const { return be64toh( extract_value<uint64_t>() ); }
